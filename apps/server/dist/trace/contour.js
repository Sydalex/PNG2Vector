...
exports.simplifyContours = simplifyContours;
exports.calculatePolygonArea = calculatePolygonArea;
exports.ensureWindingOrder = ensureWindingOrder;
 ...
 */
function isContourStart(imageData, x, y) {
    const { width, height, data } = imageData;
 ...
 */
function traceContour(imageData, startX, startY, visited) {
    const { width, height, data } = imageData;
    const points = [];
 ...
 */
function findHoles(imageData, contours) {
    const { width, height, data } = imageData;
    ...
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
            const index = y * width + x;
            const pixelIndex = index * 4;
            if (visited[index] || data[pixelIndex] !== 255)
                continue; // Not white or already visited
 ...
 */
function isHoleStart(imageData, x, y) {
    const { width, height, data } = imageData;
 ...
 */
function findContainingContour(contours, point) {
...
function simplifyPoints(points, epsilon) {
    ...
    const simplified = (0, simplify_js_1.default)(simplifyPoints, epsilon, true); // High quality
 ...
 */
function ensureWindingOrder(points, clockwise = false) {
    if (points.length < 3)
        return points;
