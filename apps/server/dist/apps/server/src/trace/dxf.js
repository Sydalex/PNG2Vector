...
exports.generateDXF = generateDXF;
exports.generateMinimalDXF = generateMinimalDXF;
exports.validateDXFOutput = validateDXFOutput;
/**
 * Generate DXF output with closed LWPOLYLINE entities
 * Ensures CAD-compatible format with proper layer structure
 */
function generateDXF(polygons, width, height, whiteFill = false) {
    const dxfContent = [];
    // DXF Header
 ...
 */
function generateDXFEntities(polygons, whiteFill) {
    const entities = [
        '0',
        'SECTION',
        '2',
    ...
    for (const polygon of polygons) {
        // Exterior contour
        const exteriorEntity = generateLWPolyline(polygon.exterior, 'VW_CLASS_Detail', (handleCounter++).toString(16).toUpperCase());
        entities.push(...exteriorEntity);
        // Hole contours
        for (const hole of polygon.holes) {
            const holeEntity = generateLWPolyline(hole, 'VW_CLASS_Detail', (handleCounter++).toString(16).toUpperCase());
            entities.push(...holeEntity);
        }
        // Generate HATCH entity for fill if enabled
        if (whiteFill) {
            const hatchEntity = generateHatchEntity(polygon, 'VW_CLASS_Fill', (handleCounter++).toString(16).toUpperCase());
 ...
 */
function generateLWPolyline(points, layer, handle) {
    if (points.length < 3)
        return [];
    const entity = [
        '0',
    ...
    for (const point of points) {
        entity.push('10', formatDXFCoordinate(point.x), // X coordinate
        '20', formatDXFCoordinate(point.y) // Y coordinate
        );
    }
    return entity;
}
 ...
 */
function generateHatchEntity(polygon, layer, handle) {
    const entity = [
        '0',
        'HATCH',
        '5',
    ...
    // Hatch style
    entity.push('75', '1', // Hatch style (normal)
 ...
 */
function generateBoundaryPath(points, pathFlag) {
    if (points.length < 3)
        return [];
    const path = [
        '92',
    ...
    // Add vertex coordinates
    for (const point of points) {
        path.push('10', formatDXFCoordinate(point.x), // X coordinate
        '20', formatDXFCoordinate(point.y) // Y coordinate
        );
    }
    path.push('97', '0' // Number of source boundary objects
    );
    return path;
}
/**
 ...
 */
function generateMinimalDXF(polygons, whiteFill = false) {
    const dxfLines = [
        '0', 'SECTION',
        '2', 'ENTITIES'
    ];
    ...
    for (const polygon of polygons) {
        ...
        if (polygon.exterior.length >= 3) {
            dxfLines.push('0', 'LWPOLYLINE', '8', 'VW_CLASS_Detail', '90', polygon.exterior.length.toString(), '70', '1' // Closed flag
            );
            for (const point of polygon.exterior) {
                dxfLines.push('10', formatDXFCoordinate(point.x), '20', formatDXFCoordinate(point.y));
            }
        }
        // Holes
        for (const hole of polygon.holes) {
            if (hole.length >= 3) {
                dxfLines.push('0', 'LWPOLYLINE', '8', 'VW_CLASS_Detail', '90', hole.length.toString(), '70', '1' // Closed flag
                );
 ...
 */
function validateDXFOutput(dxfContent) {
    const errors = [];
    const warnings = [];
